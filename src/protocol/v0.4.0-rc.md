...
- `platform` is the platform to which the instance belongs. The platform of account is defined by [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md), and the platform of item is defined by [RIP-3: Registered Indexed Items](./RIPs/RIP-3.md);
...
An External Instance is an automatically generated Instance for an external address that can be used for linked objects. For example, if users follow an RSS address, they can see updates of that RSS address.
...
| filter_platforms | If specified, returning only items with `metadata.platform` field containing the corresponding values. Multiple values are separated by commas. |
...
Prevent items or backlinks containing fields in this list from being returned, e.g. `blocklist: ["twitter"]`, in which case, items with `item.tags` and `item.metadata.platform` value of `twitter` will not be returned by AggregatedItemListURI.

###### 4.5.1.5.2. `items.x.filters.allowlist`

Only items or backlinks that contain fields in this list are allowed to be returned. If there is an allowlist, the blocklist will be invalid. For example, if `allowlist: ["twitter"]`, then only items with `item.tags` and `item.metadata.platform` value of `twitter` will be returned by AggregatedItemListURI.
...
`tags` is an array of strings. Similar to the concept of tags commonly used in personal blogs, you can add multiple tags to an article; the automatically indexed item will also have tags, indicating the type of the item, and the specific value is limited in [RIP-3: Registered Indexed Items](./RIPs/RIP-3.md).

##### 4.5.2.7. `item.authors`

`authors` is an array of InstanceURIs, indicating the authors who have produced this item, such as the author of the article, the seller of the product, etc. It can be omitted if it is the current instance address.
...
This records metadata for automatically indexed items, with specific values defined in [RIP-3: Registered Indexed Items](./RIPs/RIP-3.md).
...
This is for a note that indicates an action. This records a content before the change by this action.
...
This is for a note that indicates an action. This records a content after the change by this action.
...
The link target is an RSS3 URI, which can be an Instance URI or an Item URI. It means that the user can follow an RSS3 account or an external address such as traditional RSS feed, and comments on an RSS3 asset or an RSS3 note.
...
```json
{
    ...
    "list": [
        {
            "identifier_target": "rss3://account:0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045@evm",
            "type": "following"
        },
        {
            "identifier_target": "rss3://account:0xC8b960D09C0078c18Dcbe7eB9AB9d816BcCa8944@evm/asset/1B599C2B-4153-4DFC-8909-25AE1D51E801",
            "type": "following"
        },
        {
            "identifier_target": "rss3://note:tweet-1473342342765678605@twitter",
            "type": "following"
        },
        {
            "identifier_target": "rss3://asset:ethereum-0xacbe98efe2d4d103e221e04c76d7c55db15c8e89-5@evm",
            "type": "following"
        },
        {
            "identifier_target": "rss3://external:diygod.me%2Fatom.xml@https",
            "type": "following"
        }
    ]
}
