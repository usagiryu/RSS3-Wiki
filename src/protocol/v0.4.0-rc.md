...
The implementing program returns data and the user submits data in the form of RSS3-compliant JSON files. There are two types of files, Index File and their attached List File, which you can get an overview of by browsing through the [Use Cases](#_5-use-cases).
...
`filters` are used to filter the aggregated results returned by `identifier`. See [Filter](#_4-7-filter) for details.
...
##### 4.6.1.5. `links.filters`

This is used to filter the aggregated results returned by `identifier_back`. See [Filter](#_4-7-filter) for details.
