...
- `platform` is the platform to which the instance belongs. The platform of account is defined by [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md), and the platform of item is defined by [RIP-4: Registered Indexed Items](./RIPs/RIP-4.md);
...
| filter_platforms | If specified, returning only items with `metadata.platform` field in the filter list. Multiple values are separated by commas. |
...
###### 4.5.2.10.1. `item.metadata.platform`

The item is indexed according to a account, then the platform of this account is this platfrom.

Its value is defined by [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md).

###### 4.5.2.10.2. `item.metadata.network`

The network where this item is stored.

Its value is defined by [RIP-3: Registered Decentralized Networks](./RIPs/RIP-3.md).

###### 4.5.2.10.3. `item.metadata.proof`

This records the credentials for indexing this item, such as transaction hash and the address of a centralized platform.

###### 4.5.2.10.4. `item.metadata.from`
