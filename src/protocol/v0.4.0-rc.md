...
- `platform` is the platform to which the instance belongs. The platform of account is defined by [RIP-1: Registered Account Platforms](./RIPs/RIP-1.md), the platform of item is defined by [RIP-3: Registered Indexed Items](./RIPs/RIP-3.md);
...
The link target is an RSS3 URI, which can be an Instance URI or an Item URI, which means that the user can follow an RSS3 account, an external address such as traditional RSS feed, or comments on an RSS3 asset or an RSS3 note.
