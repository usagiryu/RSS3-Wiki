...
| link_networks | If specified, returning only links with `metadata.network` field in the filter list. Multiple values are separated by commas. |
...
| item_networks | If specified, returning only items with `metadata.network` field in the filter list. Multiple values are separated by commas. |
